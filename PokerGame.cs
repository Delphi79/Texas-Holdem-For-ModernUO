...
namespace Server.Poker
{
    ...
    public class PokerGame
    {
        ...
        private static void EventSink_Crashed( ServerCrashedEventArgs e )
        {
        ...
        public void End()
        {
            ...
            foreach ( PokerPlayer player in m_Players.Players )
            {
                player.CloseGump<PokerTableGump>();
        ...
        public PokerPlayer AssignNextTurn()
        {
            ...
            nextTurn.CloseGump<PokerBetGump>();
        ...
        public void AddPlayer( PokerPlayer player )
        {
            ...
            if ( !m_Dealer.InRange( from.Location, 8 ) )
                from.SendMessage( 0x22, "I am too far away to do that" );
    ...
    public class PokerGameTimer : Timer
    {
        ...
        protected override void OnTick()
        {
            ...
            if ( m_Game.Players.Peek() != null )
            {
                ...
                if ( m_LastPlayer.BetStart.AddSeconds( 25.0 ) <= DateTime.Now /*&& m_LastPlayer.Mobile.HasGump( typeof( PokerBetGump ) )*/ && !hasWarned )
                {
                    m_LastPlayer.SendMessage( 0x22, "You have 5 seconds left to make a choice. (You will automatically fold if no choice is made)" );
                    hasWarned = true;
                }
                else if ( m_LastPlayer.BetStart.AddSeconds( 30.0 ) <= DateTime.Now /*&& m_LastPlayer.Mobile.HasGump( typeof( PokerBetGump ) )*/ )
                {
                    PokerPlayer temp = m_LastPlayer;
                    m_LastPlayer = null;

                    temp.CloseGump<PokerBetGump>();
