    ...
    public class PokerDealer : Mobile
    {
            ...
            if ( m_JackpotWinners != null && m_JackpotWinners.Winners != null && m_JackpotWinners.Winners.Count > 0 )
            {
                ...
                foreach ( PokerPlayer m in m_JackpotWinners.Winners )
                {
                    if ( m != null && m.Mobile != null && m.Mobile.BankBox != null )
                    {
                        ...
                        World.Broadcast(1161, true, $"{m.Mobile.Name} has won the poker jackpot of {award:#,###} gold with {HandRanker.RankString(m_JackpotWinners.Hand)}");
                    }
                }
        ...
        public override void OnDoubleClick( Mobile from )
        {
            if ( !m_Active )
                ...
                from.PrivateOverheadMessage(MessageType.Regular, 0x22, true, "That is too far away.", from.NetState);
        ...
        static void EventSink_Disconnected( Mobile e )
        {
            Mobile from = e;
